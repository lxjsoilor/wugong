$(function () {

	var topHtml = '<div id="izl_rmenu" class="izl-rmenu">\
					<div id="weiChat" style="background-color:#ff8314;display:block;"\
						title="279759361" target="blank" class="btn btn-phone btn-qq">\
						<img src="data:image/png;base64," alt="" style="display:inline-block;padding:10px;">\
						<p class="text-center congHidden" style="font-size:12px">在线客服</p>\
						<div class="phone" style="overflow:hidden;background-color:#ff8314;text-align:left;padding-left:20px;"><a target="_blank" href="tel:13925817311"><span class="f_16">微信号：13925817311</span></a></div>\
					</div>\
					<div class="btn btn-phone" id="phone">\
						<img src="data:image/png;base64," alt="" style="display:inline-block;padding:10px;">\
						<p class="text-center congHidden" style="font-size:12px">咨询电话</p>\
						<div class="phone lh30" style="overflow:hidden;text-align:left;padding-left:20px;"><div class="blank10"></div><a target="_blank" href="tel:13925817311"><span class="f_16">座机：0731-83895858</span><br/><span class="f_16">手机：1330845026</span></a></div>\
					</div>\
					<div class="btn btn-top" style="display:none;">\
						<img src="data:image/png;base64," alt="" style="display:inline-block;padding-top:0px;">\
					</div>\
				</div>';
	$('body').append(topHtml);

	$("#izl_rmenu").each(function () {
		$(this).find("#phone").on('click', function () {
			$(this).find(".phone").fadeToggle("fast");
		});
		$(this).find(".btn-top").click(function () {
			$("html, body").animate({
				"scroll-top": 0
			}, "fast");
		});

		$(this).find("#weiChat").on('click', function () {
			$(this).find(".phone").fadeToggle("fast");
		});
	});
	var lastRmenuStatus = false;
	$(window).scroll(function () { //bug
		var _top = $(window).scrollTop();
		if (_top > 200) {
			$("#izl_rmenu").data("expanded", true);
		} else {
			$("#izl_rmenu").data("expanded", false);
		}
		if ($("#izl_rmenu").data("expanded") != lastRmenuStatus) {
			lastRmenuStatus = $("#izl_rmenu").data("expanded");
			if (lastRmenuStatus) {
				$("#izl_rmenu .btn-top").slideDown();
			} else {
				$("#izl_rmenu .btn-top").slideUp();
			}
		}
	});
});